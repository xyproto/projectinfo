...
func New(dir string, confAndDoc bool) (ProjectInfo, error) {
...
// const maxTokenCountPerChunk = 250000 // This constant defines the maximum token count per chunk of JSON output to avoid excessive data in one batch.

// Chunk breaks down project information into manageable JSON chunks to adhere to token limitations
func (project *ProjectInfo) Chunk(includeSourceFiles, includeConfAndDocFiles bool, maxTokenCountPerChunk int) ([]string, error) {
	var (
		chunks            []string
	...
	for _, file := range files {
		...
		if currentTokenCount+file.TokenCount > maxTokenCountPerChunk {
